...
func (*guiTest) Test_mockFileInfo() {
	...
	_ = NewApp(testRecipe, map[string]recipe.Recipe{fruit.Avocado: {
		Name: fruit.Avocado,
	}}, &workflow.TerminalFakeWorkflow{})
}

func (g *guiTest) Test_buttonPress_Pass() {
	recipes := map[string]bool{
		fruit.Blueberry: true,
	}
	b := createSubmitButton(g.mockScreen, g.mockWorkflow, g.mockFileReader, recipes, map[string]recipe.Recipe{})
	...
	g.mockWorkflow.On("SubmitShoppingList", g.mockScreen, g.mockWorkflow, g.mockFileReader, []string{fruit.Blueberry}, map[string]recipe.Recipe{}).Return(nil)
	test.Tap(b)
}
